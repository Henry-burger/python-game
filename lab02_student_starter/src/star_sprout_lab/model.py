...
class World:
    ...
    def step(self, controls: InputState, dt: float | None = None) -> None:
        ...
        # Direction intent: opposite inputs cancel naturally.s
        dx = float(controls.right) - float(controls.left)
        dy = float(controls.down) - float(controls.up)

        # Normalize non-zero direction so diagonal movement is not faster.
        length = math.hypot(dx, dy)
        if length > 0.0:
            dx /= length
            dy /= length

        # Precision movement uses half the normal speed.
        effective_speed = self.player.speed * (0.5 if controls.precision else 1.0)

        # Apply movement using the already bounded dt.
        self.player.x += dx * effective_speed * bounded_dt
        self.player.y += dy * effective_speed * bounded_dt

        # Keep the player's entire circle inside the playable area.
        min_x = self.player.radius
